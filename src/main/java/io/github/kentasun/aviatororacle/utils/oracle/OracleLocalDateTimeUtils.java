package io.github.kentasun.aviatororacle.utils.oracle;

import io.github.kentasun.aviatororacle.number.OraDecimal;
import io.github.kentasun.aviatororacle.constants.AviatorOracleConstants;
import io.github.kentasun.aviatororacle.utils.AOUtils;
...
public class OracleLocalDateTimeUtils {
    ...
    /**
     * <p>模拟 Oracle 数据库的 {@code ADD_MONTHS(date, months)} 函数：为日期加上指定月数。</p>
    ...
    /**
     * <p>模拟 Oracle 数据库的 {@code LAST_DAY(date)} 函数：返回给定日期所在月份的最后一天。</p>
    ...
    /**
     * <p>模拟 Oracle 数据库的 {@code TRUNC(date, format)} 函数：按指定格式模型截断日期。</p>
     * <p>支持的格式模型（不区分大小写）：</p>
     * <ul>
    ...
    /**
     * <p>截断到 ISO 年的第一天（ISO 第1周的周一）。</p>
     * <p>ISO 8601 规定：包含该年第一个周四的那一周是第1周，且 ISO 周以周一为起始。
     * 因此 ISO 年的第一天可能早于日历年的 1 月 1 日（最多早 3 天）。</p>
     * <p>算法：</p>
     * <ol>
